...
# `pid` never repeats, except once for a voter who is registered in two
...
table(is.na(e1$code20)) # 2024 sample: 1,431 not found in 2020

sum(is.na(e1$code20)) / sum(!is.na(e1$code))   # 14% of 2024 sample
sum(is.na(e1$code20)) / sum(!is.na(e1$code20)) # 13% of 2020 sample

table(is.na(e1$code))   # 2020 sample: 2,334 not found in 2024

sum(is.na(e1$code)) / sum(!is.na(e1$code20))   # 21% of 2020 sample
sum(is.na(e1$code)) / sum(!is.na(e1$code))     # 23% of 2024 sample
...
nrow(e2) # 1,431 left to match by other means
...
nrow(p2) # 2,334 left to match by other means
...
table(is.na(e2$code20)) # 2024 sample: 1,342 still not found in 2020
table(is.na(e2$code))   # 2020 sample: 2,245 still not found in 2024
...
nrow(e3) # 1,342 left to match by other means
...
nrow(p3) # 2,245 left to match by other means
...
table(is.na(e3$code20)) # 2024 sample: 1,331 still not found in 2020
table(is.na(e3$code))   # 2020 sample: 2,234 still not found in 2024
...
nrow(e4) # 1,331 left to match by other means
...
nrow(p4) # 2,234 left to match by other means

# [4] pid = code, fam1 (when unique to code) ------------------------------
#
# note: not risky, since family name is never missing in that subsample

# n = 1,262 remaining voters have a unique family name at the city-level
group_by(e4, code) %>%
...
nrow(e5) # 1,319 left to match by other means
nrow(p5) # 2,222 left to match by other means
...
# n = 1,294 remaining voters have (a) unique first name(s) at the city-level
group_by(e5, code) %>%
...
nrow(e6) # 1,287 left to match by other means
nrow(p6) # 2,190 left to match by other means

# [6] pid = code, first (when unique to code) -----------------------------

# n = 1,283 remaining voters have a unique date of birth at the city-level
group_by(e6, code) %>%
...
nrow(e7) # 1,274 left to match by other means
nrow(p7) # 2,177 left to match by other means

# coda --------------------------------------------------------------------

# voters from 2024 identified in 2020: n = 8,676 (includes 1 duplicate)
nrow(e) - nrow(e7)
(nrow(e) - nrow(e7)) / nrow(e) # 87% of 2024 sample (based on EU elec. lists)

# voters from 2020 identified in 2024: n = 8,675
nrow(p) - nrow(p7)
(nrow(p) - nrow(p7)) / nrow(p) # 80% of 2020 sample (dataset)
...
# largest difference in Round 1 voting behaviour (values 0-2) ~ 0.6 points
...
# small number of people changed cities between 2020 and 2024: n = 116
